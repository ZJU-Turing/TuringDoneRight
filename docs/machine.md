...
- **CPU 性能需求** 总体来说，需要高 CPU 负载的场景并不多。如果你是游戏爱好者，那确实可能需要标压的 i7/R7 及以上的处理器，否则的话日常使用其实目前的处理器都能满足需求（所以我后面说那么多处理器的事情其实是希望大家买到的产品能对得起它的价格，而不是怕同学们性能不够用）

- **内存需求** 介于 Windows11 比较臃肿，16GB 内存可能会出现不太够用的情况。出于中远期使用规划的考虑，使用不可拆卸内存的同学最好还是一步到位买 32GB 的版本，可拆卸的可以在 16G/32G 之间选择。

- **硬盘需求** 512G 的硬盘不是太够用，如果有第二个硬盘位的话可以之后来加，没有的话建议至少买 1T 的版本。

同时应当注意到的是，很多厂商在宣传是非常喜欢宣称自己的性能释放有多么强劲。但在目前的工艺下，所谓更高的性能释放其实并没有实际的意义。105w 的 4060 和 140w 的，在性能表现上其实并没有多大差异。盲目提升功耗除了让你电脑的噪音更大和更加烫手、寿命更短之外其实并没有多大意义。
...
!!! Note "前置知识"
    ...
    - 目前 CPU 型号辨别：
        - **AMD 7000 系列**：数字编号共 4 位，第一位是对应 7000 系列的 7，第二位对应同架构内型号区别，最后两位代表处理器微架构：20 对应 Zen2（对没错很老的一个架构，所以要防止上当受骗），30 对应 Zen3（也比较老了），35 对应 Zen3+，40/45 对应 Zen4。例如，笔者使用的 Ryzen 9 7940HS 表明使用的是后边的字母的代表功耗。U 为低功耗系列，热设计功耗 15w；H/HS 为标压系列，热设计功耗为 45/55W（**注意：H 和 HS 没有实质性区别，实际功耗释放由笔记本厂商设置，不用究竟是 H 还是 HS**）。值得注意的是，HX 系列为桌面端移植，采用了分离式 IOD 的设计，在增大了 L3 cache 的同时提高了功耗，降低了续航。**出于平衡的考虑，笔者不推荐在笔记本上使用 HX 处理器**。
    ...
    !!! Danger "注意"
        **Intel 的 13/14 代处理器最近爆出重大工艺缺陷，导致大量处理器出现严重的故障，在选购时请谨慎购买！**
...
!!! Note "提醒"
    - 如果你没有什么游戏需求，也不太在意性价比，但是比较在意笔记本的外观和轻便，那么华为的笔记本可能比较时候你，否则的话就一点也不推荐了
...
- **战 66 七代锐龙版** 胜在便宜且可靠，战 66 七代锐龙版价格总体来说也还算便宜，如果允许的话可以考虑直接上 Ryzen 7 7735H 的版本（据说 Zen3+ 的续航其实比 Zen4 好，反正我 Zen4 的体验是 uncore 功耗居高不下），续航、轻便、外观很不错，总的来说是值得购买的。
...
- **拯救者** 拯救者一直都有一堆子型号，说实话我也不太清楚（bushi）。总体来说评价比较一致的好，注意不同子型号的价格就行（当然续航听说不算特别好）
