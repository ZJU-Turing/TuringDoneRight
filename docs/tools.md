...
对于现在开始学习 CS 的同学，AI 和 Agent 已经不是可有可无的“加分技能”，而是必须补上的基本能力。就像搜索引擎、Git 和命令行一样，越早了解这些工具，就能越早把时间用在真正需要思考的地方。

没必要对 AI 抱有芥蒂，真正需要避免的是盲目相信和过度依赖。即使只用过豆包或 DeepSeek 也没关系，它们并不是什么只有“技术大佬”才能使用的东西，先挑一个能正常使用的工具亲自试试即可。

#### AI 是什么？

你可以先把 AI 理解成一个“读过很多东西，而且很能聊的工具”。豆包、ChatGPT、Claude、Gemini、DeepSeek 等都属于这类工具，它们背后通常是所谓的“大语言模型”。刚开始不用研究具体原理，也不必纠结哪个模型的名字听上去最厉害。
...
目前比较常见的编程 Agent 有：Codex、Claude Code、GitHub Copilot。
...
AI 会犯错，而且经常错得很有自信；Agent 更可能直接改坏文件、装错依赖或误删内容。因此，答案和代码都要自己检查，安装软件、删除文件、提交或上传内容之前也要自己确认。

不要把密码、验证码、身份证照片等隐私内容交给它们。课程是否允许使用 AI 要看老师要求，最后交上去的内容至少要保证自己看得懂、讲得清。
