...
AI 和 Agent 是现在很值得优先了解和尝试的一类工具。正确使用它们可以大大提高学习和编程的效率，帮你把时间留给真正需要思考的部分。
...
使用 AI 时，不用背各种“高级提示词”，说清自己的基础、遇到的问题和希望得到的帮助，并附上题目、代码或完整报错即可。例如：

> 我刚学 C 语言，在运行这段代码时出现了下面的报错。请先解释报错是什么意思，再带我一步步检查，暂时不要直接给出完整答案。

没听懂就让它换一种说法，回答没用就继续补充信息。更多方法可以参考 [Prompt Engineering Guide](https://github.com/dair-ai/prompt-engineering-guide)。

使用 Agent 时，打开需要处理的文件夹，说明任务、修改范围和验证方式即可。例如：

> 请检查这个项目为什么运行不了，只修改与报错有关的文件；修改后重新运行，并告诉我改了什么。不要上传或提交任何内容。

重要文件先备份或建立新的 Git 分支。入门可以参考 Datawhale 的中文开源教程 [Easy-Vibe](https://github.com/datawhalechina/easy-vibe)。

#### 用的时候记住这几件事

AI 会犯错，Agent 还可能改坏文件或误删内容，所以答案、代码和重要操作都要自己检查。不要提供密码等隐私信息，也不要直接提交生成的作业；课程是否允许使用 AI 以老师要求为准。
