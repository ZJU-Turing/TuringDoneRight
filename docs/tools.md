...
没必要对 AI 抱有芥蒂，它们也不是什么只有“技术大佬”才能使用的东西。即使之前只用过豆包或 DeepSeek 也没关系，先挑一个能正常使用的工具亲自试试即可；真正需要避免的是盲目相信和过度依赖。
...
如果想尝试编程 Agent，可以从 Codex、Claude Code、GitHub Copilot 中任选一个开始。
...
不需要专门背各种“高级提示词”，把自己的情况、遇到的问题和希望得到的帮助说清楚即可，题目、代码和完整报错也尽量一起提供。例如：

> 我刚学 C 语言，在运行这段代码时出现了下面的报错。请先解释报错是什么意思，再带我一步步检查，暂时不要直接给出完整答案。

如果回答没用，就继续补充信息；如果没听懂，就让它换一种更简单的说法。更详细的方法可以参考 [Prompt Engineering Guide](https://github.com/dair-ai/prompt-engineering-guide)。

#### 怎样使用 Agent？

打开需要处理的文件夹后，直接告诉 Agent 想做什么、哪些内容不能动，以及完成后需要怎样验证。例如：

> 请检查这个项目为什么运行不了，只修改与报错有关的文件；修改后重新运行，并告诉我改了什么。不要上传或提交任何内容。

重要文件最好提前备份，或者先建立一个新的 Git 分支。想实际体验一次，可以参考 Datawhale 的中文开源教程 [Easy-Vibe](https://github.com/datawhalechina/easy-vibe)，跟着示例用对话完成一个可以运行的小项目。

#### 用的时候记住这几件事

AI 和 Agent 归根到底只是帮助你更方便地获取知识、处理问题的工具。AI 会犯错，而且经常错得很有自信；Agent 还可能直接改坏文件、装错依赖或误删内容。因此，答案和代码都要自己检查，安装软件、删除文件、提交或上传内容之前也要自己确认。

不要把密码、验证码、身份证照片等隐私内容交给它们。课程是否允许使用 AI 要看老师要求，也不要把生成的作业、论文或代码不加检查地直接提交；最后交上去的内容，至少要保证自己看得懂、讲得清。
