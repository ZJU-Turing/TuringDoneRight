...
### 使用大语言模型
2023 年 ChatGPT 的火热程度想必大家都有所耳闻，作为 2023 年的大学生，~~你的水课作业将一定会多多少少的用到它~~，很多专业上的知识甚至是代码问题都可以向大语言模型请教（~~当然不排除它会瞎编的可能性~~）。目前比较流行的大语言模型有 ChatGPT、New Bing、Claude(最近号称超越 GPT-4 的存在)、Google Bard、文心一言、通义大模型等等。目前都是可以免费注册使用的，各别可能需要排队申请名额，每个模型没有太大的差别，需要你自己探索~
...
此外，这个网站仍在发展建设之中，欢迎同学们在未来参与到网站的建设中~（贡献指南也在这个网站里）。
